...
GFX_API Result CommandBuffer::bindGlyph(uint32_t glpyhCount, const Glyph* pGlyphs) {
  for (uint32_t i = 0; i < glpyhCount; ++i) {
    commandBuffer->state.font[static_cast<size_t>(pGlyphs[i]->createInfo.value)] = pGlyphs[i];
  }
...
GFX_API Result CommandBuffer::drawText(const char* text, uint32_t textCount,
  ...
  if (textCount > MAX_PRIMS_PER_DRAW) {
    return Result::eInvalidParameter;
  }

  if (commandBuffer->state.numDraws >= MAX_DRAWS_PER_BUFFER) {
    return Result::eInvalidParameter;
  }
  ...
  for (uint32_t i = 0; i < textCount; ++i) {
    currDraw.prims[i].glyph = commandBuffer->state.font[static_cast<size_t>(text[i])];
